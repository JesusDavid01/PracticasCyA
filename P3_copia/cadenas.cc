...
Chain::Chain(vector<Simbolo> chain, Alfabeto alphabet) : alphabet_(alphabet)
{
  ...
  for (size_t i = 0; i < chain_.size();
       i++)
  { // Recorremos la cadena y cuando encontramos un símbolo en ella
    // lo almacenamos.
    cadena += chain_[i];
    for (size_t j = 0; j < chain.size() - 1; j++)
    {
      if (cadena ==
          chain[j].Get_Symbol())
      { // Comparamos el recorrido de la cadena con
...
string Chain::Subcadenas()
{
  ...
  for (int i = 0; i < Get_Size(); i++)
  { // Recorre los simbolos uno a uno
...
string Chain::Comparacion(string cadena)
{
  ...
  { // Almaceno los valores de la string en un vector de simbolos
    temporal += cadena[i];
    aux.emplace_back(Simbolo(temporal));
    temporal = "";
  }
  for (size_t i = 0; i < chain_well_.size(); i++)
  { // Almaceno la cadena del archivo en una string
    temporal += chain_well_[i].Get_Symbol();
  }
  if (cadena == "&")
  { // Si la cadena leida por teclado es la cadena vacía, la
    // implementamos como ""
    cadena = "";
  }
  if (temporal == "" && cadena != "")
  ...
  if (temporal == "" && cadena == "")
  { // Si la cadena del archivo es igual a la cadena vacía y
    // la cadena leida por teclado no devolvemos la igualdad
    temporal += " == ";
    return temporal;
  }
  temporal = ""; // Inicializamos la string temporal para un uso futuro
  for (size_t i = 0; i < Menor(aux, chain_well_).size(); i++)
  { // Recorre los simbolos de la cadena mas pequeña
    for (size_t j = 0; j < Mayor(aux, chain_well_).size(); j++)
    { // Recorre los símbolos de la cadena mas grande
      if (aux[i].Get_Symbol() == chain_well_[i].Get_Symbol() &&
          aux.size() == chain_well_.size() && igual != 1)
      {            // Compara si los símbolos son iguales y las cadenas
                   // tienen el mismo tamaño, además de si no son diferentes
        igual = 0; // Pongo la opcion en 0, que equivale a igualdad
        resultadosub1 += aux[i].Get_Symbol(); // En caso afirmativo, guardo 
                                 // en un string resultadosub1, el mismo 
                                 // más el símbolo que obtengo de la cadena
        break;                   // Rompo el bucle
      }
      else if (Menor(aux, chain_well_)[i].Get_Symbol() ==
               Mayor(aux, chain_well_)[j].Get_Symbol())
      { // Compara si el simbolo de la cadena
        // menor y de la cadena mayor son iguales
        if ((Menor(aux, chain_well_)[i - 1].Get_Symbol() ==
                 Mayor(aux, chain_well_)[j - 1].Get_Symbol() && 
                 resultadosub1 == "") || (i == 0 && j == 0))
        { // En caso afirmativo, compara si el simbolo anterior
          ...
          resultadosub1 +=
              ...
              Menor(aux, chain_well_)[i].Get_Symbol(); // En caso afirmativo, incluye dentro del
                  ...
                  Mayor(aux, chain_well_)[j - 1].Get_Symbol() ||
                  (i != 0 || j != 0)))
        { // Compara si en caso de que no se de la
          ...
          resultadosub1 += Menor(aux, chain_well_)[i].Get_Symbol(); 
                                 // En caso afirmativo, incluye dentro del
                                 // string resultadosub1 lo que ya tiene mas el
                                 // simbolo actual de la cadena
        }
        else if (resultadosub1 != "" && (i <= j))
        {                     // Compara si el string resultadosub1 está vacío
        ...
        {                                // En caso de que no se cumplan el resto de condiciones
  ...
  if (igual != 0)
  { // Si la opcion no es equivale a igualdad
    for (size_t i = 0; i < Menor(aux, chain_well_).size(); i++)
    { // Recorre los simbolos de la cadena mas pequeña y los almacena
      // en el string temporal
      temporal += Menor(aux, chain_well_)[i].Get_Symbol();
    }
    if (resultadosub1 == temporal)
    { // Si el string resultado de la subcadena es igual a la
      // cadena mas pequeña ponemos la opcion a 2, pues sería
      // subcadena
      igual = 2;
    }
    else
    { // En caso contrario no serían iguales, por lo que ponemos la
      // opción a 1 (desigualdad)
      igual = 1;
    }
  }
  if (igual == 0)
  { // Si la opcion es la de igualdad
    for (size_t i = 0; i < chain_well_.size(); i++)
    { // Recorro la cadena simbolo a simbolo obtenida del archivo y
    ...
    { // Por último almaceno la cadena obtenida por teclado simbolo a
  ...
  { // En caso de que la opcion sea desigualdad
    for (size_t i = 0; i < chain_well_.size(); i++)
    { // Recorro la cadena simbolo a simbolo obtenida del archivo y
      // la almaceno en un string
  ...
  { // En caso de que la opcion no sea ni 1 ni 2
    for (size_t i = 0; i < Mayor(aux, chain_well_).size(); i++)
    { // Recorro la cadena mas grande simbolo a simbolo obtenida del
      // archivo y la almaceno en un string
      resultadosub += Mayor(aux, chain_well_)[i].Get_Symbol();
    }
    resultadosub += " > " + resultadosub1;   
                        // Almaceno el caracter de comparacion en el string en
...
string Chain::Concatenacion(string cadena)
{
  string resultado = "";
  for (size_t i = 0; i < chain_well_.size(); i++)
  { // Recorro la cadena obtenida del archivo simbolo a simbolo y la
    // almaceno en un string
    resultado += chain_well_[i].Get_Symbol();
  }
  resultado += cadena;       // Al string resultante le añado la cadena obtenida por teclado
...
string Chain::Potencia(int potencia)
{
  ...
  { // Eleva la cadena tantas veces como se
    // dicen en la variable potencia
    for (size_t j = 0; j < chain_well_.size(); j++)
    { // Recorro la cadena obtenida del archivo y la almaceno en un
...
string Chain::Concatenacion_Mod(string cadena)
{
