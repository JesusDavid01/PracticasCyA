...
void File::isString() {
  ...
  std::cout << "El DFA tiene " << num_nodos << " de estados" << std::endl;
  std::cout << "El estado de arranque es " << nodeS << std::endl;
  for (size_t j = 2; j < dna_.size(); j++) {  // Bucle para obtener las cadenas por tamaño
    actual_node = nodeS;
    std::cout << "TRANSICIONES CON CADENAS DE TAMAÑO " << j << std::endl;
    for (size_t i = 0; i < dna_.size(); i++) {  // Iteramos cada uno de los simbolos de la cadena
      ...
      std::cout << "Símbolo con el que se realiza la transición: " << aux << std::endl;
      std::cout << "Estado final: " << actual_node << std::endl << std::endl;
      if (result.size() == j &&
