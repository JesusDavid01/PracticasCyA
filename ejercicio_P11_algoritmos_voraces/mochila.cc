      ...
      switch (i) {
        ...
        default:
          if (aux_numbers[j] == space && contador == 0) {
            peso_[m] = stoi(numbers);
            ++m;
            contador++;
            numbers = "";
          } else if (aux_numbers[j] == space && (contador >= 1 && contador <=4)) {
            indices_[i][k] = stoi(numbers);
            contador++;
            numbers = "";
            ++k;
          } else {
  ...
  for (int i = 1; i <= numero_objetos_; ++i) {
    if ((peso_dinamico + peso_[posicion_[i]]) < peso_mochila_) {
      ...
      for (int j = 1; j <= size; j++) {
        final_beneficio += indices_[posicion_[i]][j];
      }
    } else {
